...
当前以 `RainGui/build.bat`、`RainGuiDx12HookTest/build.bat`
和下方 DX12Hook 备注为准。
...
DX12 游戏 Hook 按这条路径处理，和当前构建产物一致。
...
用于验证 `RainGui.dll + DX12Hook` 这条链路是否能正常装上。
...
2. `RainGuiDx12HookTest.dll` 仅用于验证 hook，
   正式接入统一直接链接 `RainGui.lib`
