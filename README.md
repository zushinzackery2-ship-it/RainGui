...
- 当前主线的 `DX11Hook + RainGui.dll / RainGui.lib` 方案
- 当前主线的 `DX12Hook + RainGui.dll / RainGui.lib` 方案
...
- 当前主钩点为 `IDXGISwapChain::Present`（vtable[8]）
- 在 `Present` 阶段处理资源初始化 / 重建、WndProc hook、device lost 和安全卸载
...
## 当前主线接法：DX11Hook / DX12Hook

下面保留 DX12 示例，DX11 接法完全同构，只需要把 `Dx12` 类型名和导出名替换为 `Dx11`。
...
`RainGuiDx11HookTest/` 和 `RainGuiDx12HookTest/` 都是独立测试载荷。
