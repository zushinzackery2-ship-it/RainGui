...
namespace RainGuiDx11Hook
{
    void FillDefaultDesc(RainGuiDx11HookDesc* desc)
    {
        ...
        RainGuiDx11HookInternal::FillDefaultDesc(*desc);
    }
    ...
    bool Init(const RainGuiDx11HookDesc* desc)
    {
        ...
        RainGuiDx11HookInternal::FillDefaultDesc(g_state.desc);
        if (desc)
        {
            g_state.desc = *desc;
        }
    ...
    bool IsInstalled()
    {
        return RainGuiDx11HookInternal::g_state.installed;
    }

    bool IsReady()
    {
        const auto& state = RainGuiDx11HookInternal::g_state;
        return state.installed && state.backendReady && !state.deviceLost;
    }

    const RainGuiDx11HookRuntime* GetRuntime()
    {
        if (!RainGuiDx11HookInternal::g_state.installed)
        {
            return nullptr;
        }

        return &RainGuiDx11HookInternal::g_state.runtime;
    }
}
