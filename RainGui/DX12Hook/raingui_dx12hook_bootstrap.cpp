...
namespace RainGuiDx12Hook
{
    void FillDefaultDesc(RainGuiDx12HookDesc* desc)
    {
        ...
        RainGuiDx12HookInternal::FillDefaultDesc(*desc);
    }

    bool Init(const RainGuiDx12HookDesc* desc)
    {
        using namespace RainGuiDx12HookInternal;
        ...
        RainGuiDx12HookInternal::FillDefaultDesc(g_state.desc);
        if (desc)
        {
            g_state.desc = *desc;
        }
        ...
        InitializeSRWLock(&g_state.rainGuiLock);
        ResetRuntime();
        ...
        g_state.defaultDebugVisible = g_state.desc.enableDefaultDebugWindow && g_state.desc.startVisible;
    ...
    void Shutdown()
    {
        ...
        g_state.defaultDebugVisible = false;
    ...
    bool IsInstalled()
    {
        return RainGuiDx12HookInternal::g_state.installed;
    }

    bool IsReady()
    {
        const auto& state = RainGuiDx12HookInternal::g_state;
        return state.installed && state.backendReady && !state.deviceLost;
    }

    const RainGuiDx12HookRuntime* GetRuntime()
    {
        if (!RainGuiDx12HookInternal::g_state.installed)
        {
            return nullptr;
        }

        return &RainGuiDx12HookInternal::g_state.runtime;
    }
}
