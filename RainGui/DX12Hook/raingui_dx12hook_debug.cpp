...
namespace RainGuiDx12HookInternal
{
    static void RenderDefaultDebugWindow()
    {
        ...
        RainGui::SetNextWindowSize(ImVec2(430.0f, 250.0f), RainGuiCond_FirstUseEver);

        bool keepVisible = g_state.defaultDebugVisible;
        if (RainGui::Begin("RainGui DX12Hook", &keepVisible, 0))
        {
            RainGui::Text("DX12 hook is active");
            RainGui::Separator();
            ...
            RainGui::Text("Queue: 0x%p", g_state.runtime.commandQueue);
            RainGui::Spacing();
